...
static inline bool _operator_eff_includes_non_conditional(const SimpleOperator& o,
    ...
    for (unsigned j = 0; j < prepost.size(); j++) {
        for(unsigned int k = 0; k < lmp->vars.size(); k++) {
            if(double_equals(prepost[j].var, lmp->vars[k]) && double_equals(prepost[j].post, lmp->vals[k]))
...
LandmarksGraph::LandmarksGraph() : landmarks_count(0),use_external_inconsistencies(false),reasonable_orders(false) {
...
bool LandmarksGraph::disj_landmark_exists(const set
            <pair<int, int> >& lm) const {
...
bool LandmarksGraph::exact_same_disj_landmark_exists(const set
            <pair<int, int> >& lm) const {
...
LandmarkNode& LandmarksGraph::landmark_add_simple(const pair<int, int>& lm) {
    ...
    vector<int> vars;
    vector<int> vals;
    //if(g_variable_types[lm.first] == logical){
...
LandmarkNode& LandmarksGraph::landmark_add_disjunctive(const set
            <pair<int, int> >& lm) {
    vector<int> vars;
    vector<int> vals;
...
void LandmarksGraph::generate() {
    ...
    if(reasonable_orders) {
        ...
        approximate_reasonable_orders(false); // no obident reasonable orders!
    }
}
    ...
    if(myfile.is_open()) {
        ...
        for(int j = 0; j < g_variable_name.size(); j++) {
            ...
            for(int k = 0; k < g_variable_domain[j]; k++)
                inconsistent_facts[j].push_back(set
                                                <pair<int, int> >());
        }
...
bool LandmarksGraph::relaxed_task_solvable(vector<vector<int> >& lvl_var,
    ...
    if(compute_lvl_op) {
        lvl_op.resize(g_simpleoperators.size());
        for(int i = 0; i < g_simpleoperators.size(); i++) {
            const SimpleOperator& op = get_operator_for_lookup_index(i);
            lvl_op[i] = map<pair<int, int>, int>();
            const vector<PrePost>& prepost = op.get_pre_post();
            for(unsigned j = 0; j < prepost.size(); j++)
                lvl_op[i].insert(make_pair(make_pair(prepost[j].var, (int)prepost[j].post), INT_MAX));
        }
    }
    lvl_var.resize(g_variable_name.size());
    for (unsigned var = 0; var < g_variable_name.size(); var++) {
        if(g_variable_types[var] == logical) // just for logical variable, huym 10-08-11
             lvl_var[var].resize(g_variable_domain[var], INT_MAX);
    }
    // Extract propositions from "exclude"
    hash_set<const SimpleOperator *, hash_operator_ptr> exclude_ops;
    vector<pair<int, int> > exclude_props;
    if(exclude != NULL) {
        for(int op = 0; op < g_simpleoperators.size(); op++) {
            if(_operator_eff_includes_non_conditional(g_simpleoperators[op], exclude))
                exclude_ops.insert(&g_simpleoperators[op]);
        }
    ...
    for(int i = 0; i < g_goal.size(); i++)
        if(lvl_var[g_goal[i].first][(int)g_goal[i].second] == INT_MAX) {
            return false;
        }
    return true;
}
...
void LandmarksGraph::generate_operators_lookups() {
    ...
    for (unsigned i = 0; i < g_variable_domain.size(); i++) {
		...
		operators_pre_lookup[i].resize(0);
		operators_eff_lookup[i].resize(0);
	}
    }
    for (unsigned i = 0; i < g_simpleoperators.size(); i++) {
        const SimpleOperator& op = get_operator_for_lookup_index(i);

        const vector<PrePost>& prepost = op.get_pre_post();
        bool no_pre = true;
        for (unsigned j = 0; j < prepost.size(); j++) {
	    if(g_variable_types[prepost[j].var] != logical) continue;
            if(!double_equals(prepost[j].pre, -1)) {
                no_pre = false;
                operators_pre_lookup[prepost[j].var][(int)prepost[j].pre].push_back(i);
            }
            operators_eff_lookup[prepost[j].var][(int)prepost[j].post].push_back(i);
        }
        const vector<Prevail>& prevail = op.get_prevail();
        for(unsigned j = 0; j < prevail.size(); j++) {
            no_pre = false;
            if(!double_equals(prevail[j].prev, -1) && g_variable_types[prevail[j].var] == logical)
            operators_pre_lookup[prevail[j].var][(int)prevail[j].prev].push_back(i);
        }
        if(no_pre)
...
bool LandmarksGraph::effect_always_happens(const vector<PrePost>& prepost,
        ...
        for(unsigned i = 0; i < prepost.size(); i++) {
            ...
            if(effect_vars.find(prepost[i].var) != effect_vars.end()) {
                ...
                if(!double_equals(old_eff, prepost[i].post)) {
        ...
        for(; it != effect_conditions.end(); ++it) {
            ...
            for(unsigned int j = 0; j < conds.size(); j++) {
                if(unique_conds.find(conds[j].first) != unique_conds.end()) {
                    ...
                    set
                        <int>& the_set = unique_conds.insert
                                         (make_pair(conds[j].first, set
                                                        <int>())).first->second;
...
bool LandmarksGraph::inconsistent(const pair<int,int>& a,
...
bool LandmarksGraph::interferes(const LandmarkNode* node_a, const LandmarkNode* node_b) const {
    ...
    */
    ...
    for(unsigned i = 0; i < ops.size(); i++) {
        ...
        const vector<PrePost>& prepost = op.get_pre_post();
        set
            <pair<int, int> > trivially_conditioned_effects;
        ...
        for(unsigned i = 0; i < prepost.size(); i++) {
            if(prepost[i].cond.empty() && prepost[i].var != a.first) {
                next_eff.insert(make_pair(prepost[i].var, (int)prepost[i].post));
            } else if(testing_for_trivial_conditions && trivial_conditioned_effects_found &&
                      trivially_conditioned_effects.find(make_pair(prepost[i].var, (int)prepost[i].post)) !=
...
inline static bool _in_goal(const pair<int,int>& l) {
    for(vector<pair<int, double> >::const_iterator start = g_goal.begin();
            start != g_goal.end(); start++)
        if(start->first == l.first && double_equals(start->second, l.second)) // l \subseteq goal
            return true;
    ...
    for(set<LandmarkNode*>::iterator it = nodes.begin(); it != nodes.end(); it++) {
        ...
        if(!obedient_orders &&_in_goal(node_prop)) {
            ...
            for(hash_map<LandmarkNode*, edge_type, hash_pointer>::iterator
                    it = node_p->children.begin();
                    it != node_p->children.end();
                    it++) {
            ...
            // with node_p.
            for(hash_set<LandmarkNode*, hash_pointer>
                    ::iterator
                    it3 = interesting_nodes.begin();
                    it3 != interesting_nodes.end();
                    it3++) {
                if((*it3)->disjunctive)
                    continue;
...
void LandmarksGraph::dump() const {
    for(set<LandmarkNode*>::const_iterator it= nodes.begin(); it != nodes.end(); it++) {
        LandmarkNode* node_p = *it;
        dump_node(node_p);

        for(hash_map<LandmarkNode*, edge_type, hash_pointer>::const_iterator parent_it
                = node_p->parents.begin();
        ...
        for(hash_map<LandmarkNode*, edge_type, hash_pointer>
                ::const_iterator
...
/*
//caidb: dump "before order"
void LandmarksGraph::dump_before() const {
    for(hash_set<LandmarkNode*, hash_pointer>::const_iterator it= nodes.begin(); it != nodes.end(); it++) {
        LandmarkNode* node_p = *it;
        dump_node(node_p);

        for(hash_map<LandmarkNode*, edge_type, hash_pointer>::const_iterator parent_it
                = node_p->parents_before.begin();
                parent_it != node_p->parents_before.end();
                parent_it++) {
            const edge_type& edge = parent_it->second;
            const LandmarkNode* parent_p = parent_it->first;
            cout << "\t\t<-_";
        ...
        for(hash_map<LandmarkNode*, edge_type, hash_pointer>
                ::const_iterator
                child_it = node_p->children_before.begin();
                child_it != node_p->children_before.end();
                child_it++) {
            const edge_type& edge = child_it->second;
            const LandmarkNode* child_p = child_it->first;
            cout << "\t\t->_";
...
void LandmarksGraph::mk_acyclic_graph() {
    //set<LandmarkNode*> acyclic_node_set(number_of_landmarks());
...
int LandmarksGraph::loop_acyclic_graph(LandmarkNode& lmn, hash_set<LandmarkNode*, hash_pointer>& acyclic_node_set) {
    assert(acyclic_node_set.find(&lmn) == acyclic_node_set.end());
    int nr_removed = 0;
    list<pair<LandmarkNode*, edge_type> > path;
    //set<LandmarkNode*> visited = set<LandmarkNode*>(number_of_landmarks());
