...
class LandmarksGraphNew : public LandmarksGraph {
    ...
    void get_greedy_preconditions_for_lm(const LandmarkNode *lmp, const SimpleOperator& o,
    ...
    void found_lm_and_order(const pair<int, int> a, LandmarkNode& b,
                            edge_type t);
    void found_disj_lm_and_order(const set
                                     <pair<int, int> >a, LandmarkNode& b,
                                     edge_type t);
    void approximate_lookahead_orders(const vector<vector<int> >& lvl_var,
                                      LandmarkNode* lmp);
    static bool domain_connectivity(const pair<int, int>& landmark, const set<int>& exclude);
