...
struct LocalAssignment {
    DomainTransitionGraph *prev_dtg; // the dtg of the variable coressponding to  <var, value> condition
    int local_var;
    double value;
    int var; // for state(var_no) fop= state(var)
    int global_var; // for order version,huym
    assignment_op fop;
    ...
    //for ordered version, huym, 10-08-24
    LocalAssignment(DomainTransitionGraph *_prev_dtg, int _global_var, int _local_var, double _value) :
        prev_dtg(_prev_dtg), global_var(_global_var), local_var(_local_var), value(_value) {
            var = -1;                                   
    } 
    void dump() const;
};
