...
enum edge_type {b_n = 6, b_r = 5, n = 4, gn = 3, r = 1, o_r = 0, ln = 2};
...
public:
	...
	bool operator<(const LandmarkNode& other) {
		if(vars.size() < other.vars.size())
			return true;
		else if(vars.size() == other.vars.size()) {
			for(int i = 0; i < other.vars.size(); i++) {
				if(vars[i] < other.vars[i])
					return true;
				else if(vars[i] == other.vars[i] && !double_equals(vals[i], other.vals[i]) && vals[i] < other.vals[i])
					return true;
			}
		}
		return false;
	...
	class compare_nodes {
	public:
		bool operator()(LandmarkNode* n2, LandmarkNode* n3) {
			return (*n2) < (*n3);
		}
	};*/ //by huym
    ...
    set<pair<int, int> > forward_orders;
    /*
    //caidb: data begin
    map<LandmarkNode*, edge_type, compare_nodes> parents_before;
    map<LandmarkNode*, edge_type, compare_nodes> children_before;
    
    set<LandmarkNode*> ancestors;
    
    bool ancestors_collected;
    
    set<LandmarkNode*> ancs_in_cond;
    //dynamic member.
    //for lmn_p, lmn_q is in ancs_in_cond(lmn_p) iff. 
    //lmn_q <* lmn_p holds and lmn_q is in the same precondition
    //as lmn_p in.
    //This is temporary data for determining context(lmn_p): best_anc; 
    
    LandmarkNode* best_anc; 
    //dynamic member: in a give precond, 
    //the nearest ancestor(s) with the lowest "index"
    //caidb: data end
    */
    bool is_goal() const {
        return in_goal;
    ...
    bool is_true_in_state(const TimeStampedState& state) const {
        if(!disjunctive)
            return double_equals(state[vars[0]], vals[0]);
        else {
            bool is_true = false;
            for(unsigned int i = 0; i < vars.size(); i++)
                if(double_equals(state[vars[i]], vals[i])) {
                    is_true = true;
                    break;
                }
            return is_true;
...
public:
	bool operator()(LandmarkNode* n2, LandmarkNode* n3) {
		//
		//assert(!(n2->disjunctive) && !(n3->disjunctive));
		//if((n2->vars)[0] < (n3->vars)[0])
		//	return true;
		//else if((n2->vars)[0] == (n3->vars)[0] && (n2->vals)[0] < (n3->vals)[0])
			return true;
		//return false;
		//
...
public:
    ...
    inline const vector<int>& get_operators_including_eff(const pair<int, int>& eff) const {
	     // if(eff.second < operators_eff_lookup[eff.first].size() && eff.second >= 0)
	...
	// if(pre.second < operators_pre_lookup[pre.first].size() && pre.second >= 0)
                  return operators_pre_lookup[pre.first][pre.second];
        // else return 0;
    }
    inline const SimpleOperator& get_operator_for_lookup_index(int op_no) const {
	    // if(op_no >= 0 && op_no < g_simpleoperators.size()){
		 const SimpleOperator& op = g_simpleoperators[op_no];
                 return op;
	    // }
	    // else NULL;
...
protected:
   ...
   hash_map<pair<int, int>, LandmarkNode*, hash_int_pair> simple_lms_to_nodes;
	...
	// dunbo, this does not bring randomness
    ...
    friend class PCGraph; // dunbo: 
/*
public:    
    //caidb
    virtual void set_best_anc(set<LandmarkNode*>& lmns_in_precond) = 0;
    virtual void generate_before() = 0;
    void dump_before() const; //dump before order
