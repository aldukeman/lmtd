...
bool _possibly_fires(const vector<Prevail>& prevail,
    ...
    for(int j = 0; j < prevail.size(); j++)
        if(lvl_var[prevail[j].var][(int)prevail[j].prev] == INT_MAX)
            return false;
    return true;
}

map<int, int> _intersect(const map<int, int>& a,
                              const map<int, int>& b) {
    if(a.size() > b.size())
        return _intersect(b, a);
    map<int, int> result;
    for(map<int, int>::const_iterator it1 = a.begin(); it1 != a.end(); it1++) {
        map<int, int>::const_iterator it2 = b.find(it1->first);
...
bool _possibly_reaches_lm(const SimpleOperator& o,
                          const vector<vector<int> >& lvl_var,
    ...
    for(unsigned i = 0; i < prevail.size(); i++)
        if(lvl_var[prevail[i].var][(int)prevail[i].prev] == INT_MAX)
            return false;
    ...
    for(unsigned i = 0; i < prepost.size(); i++) {
        ...
        // Otherwise, operator is not applicable
        if(prepost[i].pre != -1)
            if(lvl_var[prepost[i].var][(int)prepost[i].pre] == INT_MAX)
                return false;
        ...
        for(unsigned int j = 0; j < lmp->vars.size(); j++)
            if(prepost[i].var == lmp->vars[j] && double_equals(prepost[i].post, lmp->vals[j])) {
                correct_effect = true;
                break;
            }
        if(correct_effect)
...
void LandmarksGraphNew::get_greedy_preconditions_for_lm(const LandmarkNode *lmp,
        const SimpleOperator& o,
        map<int, int>& result) const {
    ...
    for(unsigned j = 0; j < prepost.size(); j++)
        if(!double_equals(prepost[j].pre, -1)) {
            assert(prepost[j].pre < g_variable_domain[prepost[j].var]);
            result.insert(make_pair(prepost[j].var, (int)prepost[j].pre));
        } else if(g_variable_domain[prepost[j].var] == 2)
            for(int i = 0; i < lmp->vars.size(); i++)
                if(lmp->vars[i] == prepost[j].var &&
                        !double_equals((*g_initial_state)[prepost[j].var], lmp->vals[i])) {
                    result.insert(make_pair(prepost[j].var, (int)(*g_initial_state)[prepost[j].var]));
                    break;
                }
    const vector<Prevail> prevail = o.get_prevail();
    for(unsigned j = 0; j < prevail.size(); j++)
        result.insert(make_pair(prevail[j].var, (int)prevail[j].prev));
    // Check for lmp in conditional effects
    set<int> lm_props_achievable;
    for(unsigned j = 0; j < prepost.size(); j++)
        for(int i = 0; i < lmp->vars.size(); i++)
            if(lmp->vars[i] == prepost[j].var && double_equals(lmp->vals[i], prepost[j].post))
    ...
    for(; it != lm_props_achievable.end(); ++it) {
        for(unsigned j = 0; j < prepost.size(); j++) {
            if(!init && intersection.empty())
                break;
            map<int, int> current_cond;
            if(lmp->vars[*it] == prepost[j].var &&
                    double_equals(lmp->vals[*it], prepost[j].post))
...
/*
int LandmarksGraphNew::min_cost_for_landmark(LandmarkNode* bp, vector<vector<int> >& lvl_var) {
	...
	if(g_variable_types[b.first] != logical) continue;
        const vector<int>& ops = get_operators_including_eff(b);
        for (unsigned i = 0; i < ops.size(); i++) {
            const SimpleOperator& op = get_operator_for_lookup_index(ops[i]);
...
void LandmarksGraphNew::found_disj_lm_and_order(const set
            <pair<int, int> > a,
            LandmarkNode& b,
            edge_type t) {
    ...
    for(set<pair<int, int> >::iterator it = a.begin(); it != a.end(); ++it) {
        if(double_equals((*g_initial_state)[it->first], it->second)) {
...
void LandmarksGraphNew::compute_shared_preconditions(map<int, int>& shared_pre,
...
compute_disjunctive_preconditions(vector<set<pair<int, int> > >& disjunctive_pre,
    ...
    int no_ops = 0;
    map<int, vector<pair<int, int> > > preconditions; // maps from
    ...
    for (unsigned i = 0; i < ops.size(); i++) {
        ...
        if(_possibly_reaches_lm(op, lvl_var, bp)) {
            ...
            map<int, int> next_pre;
            get_greedy_preconditions_for_lm(bp, op, next_pre);
            for(map<int, int>::iterator it = next_pre.begin(); it != next_pre.end(); it++) {
    ...
    for(map<int, vector<pair<int, int> > >::iterator it
        ...
        if(it->second.size() == no_ops) {
            set
                <pair<int, int> > pre_set; // the set gets rid of duplicate predicates
...
//caidb: order all facts;
// 1): set goals as landmarks
// 2): add reachable non-goal facts as landmarks;
// 3): compute order for the above facts
// 4): add all unreachable facts as landmarks, on which no order is defined.
//(when try to optimize, consider relevant facts only? Not feasible)

// order only landmarks
// 1): set goals as landmarks;
// 2): identify more landarks and compute the order for them
// 3): insert all facts that are not landmarks into the landmarks graph
// Note: step 3) will not affect the results of resonable ordering, but merely
// the time-cost to do reasonable ordering. But, maybe {p,q} is a disjunctive landmark
// and now step 3) add {p} and {q} as two simple landmarks (nodes).
void LandmarksGraphNew::generate_landmarks() {
  //  cout << "Generating landmarks using the RPG/SAS+ approach\n";
    // 1) add goals
    map<int, int> goals; //caidb:  
    for(unsigned i = 0; i < g_goal.size(); i++) {
        LandmarkNode& lmn = landmark_add_simple(make_pair(g_goal[i].first, (int)g_goal[i].second));
        lmn.in_goal = true;
        open_landmarks.push_back(&lmn);
        
        goals[g_goal[i].first] = (int)g_goal[i].second;
    }
	    
	// order only landmarks
	if(g_before_order_option == 4 || g_before_order_option == 5) {
		//cout << "Order only landmarks" << endl;
        // 2): identify more landmarks and compute the order on them
		while(!open_landmarks.empty()) {
			...
			if(!bp->is_true_in_state(*g_initial_state)) {
				...
				for(map<int, int>::iterator it = shared_pre.begin(); it != shared_pre.end(); it++) {
		...
		//	        for(int value = 0; value < num_values; value++) {  
                //                     // don't skip the top level goals      	
        	//	             /*if(goals.count(var_no)) { //skip top level goals
        	//		     if(value == goals[var_no])
        	//			   continue;
		//		     }*/
		//		     if(!landmark_exists(make_pair(var_no, value))) {
		//			    LandmarkNode& lmn = landmark_add_simple(make_pair(var_no, value));
		//			    lmn.in_goal = false;
		//		     }
		//	        }
                //        }
		...
		//cout << "Order all facts " << endl;

		// 2) add all reachable non-goal facts
		// first collect reachable facts
		vector<vector<int> > reachable_lvl_var;
		vector<map<pair<int, int>, int> > reachable_lvl_op;    
		  
		if( !relaxed_task_solvable(reachable_lvl_var, reachable_lvl_op, true, NULL) ) {
			//"NULL" indicates "exclude nothing"
			cout << "Task is unsolvable! \n";
    		//cout << "Reported by LandmarksGraphNew::relaxed_task_solvable( ) \n";		
		}
    
		int num_unreached_facts = 0;
		int num_variables = g_variable_domain.size();
			...
			if(g_variable_types[var_no] == logical){
			     ...
			     for(int value = 0; value < num_values; value++) {        	
        				   ...
        				   continue;
        		       	   ...
        		       	   //skip unreachable facts        	
        		           if(reachable_lvl_var[var_no][value] == INT_MAX) {
        			         unreachable_facts.push_back(make_pair(var_no, value));
        			         //cout << "unreachable fact: (" << g_variable_name[var_no] << "=" << value << ")" << endl;
        			         ++num_unreached_facts;
        			             continue;
        			    }
                                    if(!landmark_exists(make_pair(var_no, value))){  //added by huym
                               		    LandmarkNode& lmn = landmark_add_simple(make_pair(var_no, value));
        		      		    lmn.in_goal = false;
        		      		    open_landmarks.push_back(&lmn);
                        	    } //
		...
		//cout << num_unreached_facts << " unreachable facts found.\n";
		// 3) compute the orde on all the above facts
		while(!open_landmarks.empty()) {
			LandmarkNode* bp = open_landmarks.front();
            		open_landmarks.pop_front();
			assert(bp->forward_orders.empty());
			...
			if(!bp->is_true_in_state(*g_initial_state)) {       
				...
				/*caidb: since all facts are set as landmarks, we don't need disjunctive ones
				// Process achieving operators again to find disj. LMs
				vector<set<pair<int, int> > > disjunctive_pre;
				compute_disjunctive_preconditions(disjunctive_pre, lvl_var, bp);
				for(int i = 0; i < disjunctive_pre.size(); i++)
            		//caidb: it was 5
					if(disjunctive_pre[i].size() < 2) { // We don't want disj. LMs to get too big
						found_disj_lm_and_order(disjunctive_pre[i], *bp, gn);
					}
				*/
			}
		...
		add_lm_forward_orders();
                /*
		// 4) add the unreachable facts as nodes only, there are no relation defined on them.
		for(int i = 0; i < unreachable_facts.size(); i++) {
    		LandmarkNode& lmn = landmark_add_simple(unreachable_facts[i]);
			lmn.in_goal = false;
		}*/
	}
    ...
    // debug	
    /*
	long number_of_lmns = 0;
	int num_variables = g_variable_domain.size();
	for(int var_no = 0; var_no < num_variables; var_no++) {
		number_of_lmns += g_variable_domain[var_no];		
	}
	assert(number_of_lmns == nodes.size());
	*/
}
...
void LandmarksGraphNew::approximate_lookahead_orders(const vector<vector<int> >& lvl_var,
    ...
    //caidb: unreached contains all unreached values before LM, 
    //but not contain LM, as LM is under condsideration.
...
bool LandmarksGraphNew::domain_connectivity(const pair<int, int>& landmark,
    ...
    */
    assert(!double_equals(landmark.second, (*g_initial_state)[landmark.first])); // no initial state landmarks
    ...
    if(exclude.find((int)((*g_initial_state)[landmark.first])) != exclude.end())
        return false;
    list<int> open;
    set<int> closed;
    closed = exclude;
    open.push_back((int)((*g_initial_state)[landmark.first]));
    closed.insert((int)((*g_initial_state)[landmark.first]));
    ...
    */
    for(int i = 0; i < g_variable_domain.size(); i++)
		...
		for(int j = 0; j <  g_variable_domain[i]; j++) {
            		...
            		bool insert = true;
			//cout<<"lmp->vars   "<<lmp->vars.size()<<"  "<<lmp->vals.size()<<endl;
					...
					// cout<<"get the operators"<<endl;
                   			 ...
                   			 const vector<int>& reach_lm = get_operators_including_eff(lm);
                                         //cout<<"reach_ij  "<<reach_ij.size()<<"  "<<reach_lm.size()<<endl;
...
void LandmarksGraphNew::add_lm_forward_orders() {
    ...
    for(node_it = nodes.begin(); node_it != nodes.end(); ++node_it) {
        ...
        for(set<pair<int, int> >::iterator it2 = node.forward_orders.begin();
