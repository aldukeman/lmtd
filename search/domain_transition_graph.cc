...
void DomainTransitionGraphSymb::read_data(istream &in) {
	...
	for(int i = 0; i < trans_count; i++) {
		...
		all_prevails.push_back(LocalAssignment(prev_dtg, ccg_parent, (double)val, cond_type)); 
	    }
...
void DomainTransitionGraphSymb::dump() const {
	...
	for(int j = 0; j < transitions.size(); j++) {
		...
		vector<LocalAssignment> effect = labels[k]->effect;
		for(int l = 0; l < effect.size(); l++) {
		    cout << "   " << effect[l].local_var << ":"<<effect[l].fop<<"   "<<effect[l].var<<"   "
			    << effect[l].value << endl;
		}
	    }
	}
