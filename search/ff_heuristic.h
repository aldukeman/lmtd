...
struct UnaryOperator {
    ...
    UnaryOperator(const std::vector<Proposition *> &pre, Proposition *eff,
		  const SimpleOperator *the_op, int base)
...
class FFHeuristic : public Heuristic {
    ...
    typedef __gnu_cxx::hash_set<const SimpleOperator *, hash_operator_ptr> RelaxedPlan;
    ...
    void build_unary_operators(const SimpleOperator &op);
    void simplify();

    void setup_exploration_queue(const TimeStampedState &state, 
				 const std::vector<std::pair<int, int> >& excluded_props,
				 const __gnu_cxx::hash_set<const SimpleOperator *, 
				 hash_operator_ptr>& excluded_ops,
				 bool use_h_max);
    inline void setup_exploration_queue(const TimeStampedState &state, bool h_max) {
	std::vector<std::pair<int, int> > excluded_props;
	__gnu_cxx::hash_set<const SimpleOperator *, hash_operator_ptr> excluded_ops;
	setup_exploration_queue(state, excluded_props, excluded_ops, h_max);
    }
    void relaxed_exploration(bool use_h_max, bool level_out);
    void prepare_heuristic_computation(const TimeStampedState& state, bool h_max);
    void collect_relaxed_plan(Proposition *goal, RelaxedPlan &relaxed_plan, const TimeStampedState &state);

    int compute_hsp_add_heuristic();
    int compute_hsp_max_heuristic();
    int compute_ff_heuristic(const TimeStampedState &state);

    void collect_ha(Proposition *goal, RelaxedPlan &relaxed_plan, const TimeStampedState &state);

    void enqueue_if_necessary(Proposition *prop, int cost, int depth, UnaryOperator *op, 
			      bool use_h_max);
    
    std::vector<const SimpleOperator*> ff_preferred_operators;

protected:
    virtual double compute_heuristic(const TimeStampedState &state);
public:
    int get_lower_bound(const TimeStampedState &state);
    ...
    void compute_reachability_with_excludes(std::vector<std::vector<int> >& lvl_var,
    					std::vector<std::map<pair<int, int>, int> >& lvl_op,
    					bool level_out,
    					const std::vector<std::pair<int, int> >& excluded_props,
    					const __gnu_cxx::hash_set<const SimpleOperator*, hash_operator_ptr>& excluded_ops,
					    bool compute_lvl_ops);
					    
    std::vector<const SimpleOperator *> exported_ops; // only needed for landmarks count heuristic ha
    int plan_for_disj(std::vector<std::pair<int, int> >& disj_goal, const TimeStampedState& state);
    FFHeuristic(bool use_cache=false);
    ~FFHeuristic();

     void set_ff_preferred(const SimpleOperator* op);
