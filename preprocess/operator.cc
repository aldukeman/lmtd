...
Operator::Operator(istream &in, const vector<Variable *> &variables) {
    ...
    compoperator cop;
    in >> cop >> varNo; // = / < />  number
    duration_cond = DurationCond(cop, variables[varNo]); //what's this for?
    int count;
    ...
    in >> count; // number of pre_post_start conditions
                 // that is the start effect with (when) conditions
    for(int i = 0; i < count; i++) {
	int eff_conds;
	...
	int varNo;
	in >> varNo;
	if(variables[varNo]->is_functional()) { // for the numeric variables, that's the compariable variables, they are added for temp.
	    foperator assignop;
	    int varNo2;
	    in >> assignop >> varNo2;
	    if(eff_conds)
	...
	} else {                               // for the real variables,that is the compariable graph's no-indegree node.
	    int val, newVal;
	    in >> val >> newVal;
	    if(eff_conds)
    ...
    in >> count; // number of pre_post_end conditions
                 // that is the end effect with (when) condition
    for(int i = 0; i < count; i++) {
	int eff_conds;
	vector<EffCond> ecs_start;
	vector<EffCond> ecs_overall;
...
void Operator::write_pre_posts(ostream &outfile, const vector<PrePost> &pre_posts) const {
  ...
  for (int i = 0; i < pre_posts.size(); i++) {
   ...
   // cout<< "the detail  " << pre_posts[i].var->get_level() << " " << pre_posts[i].pre << " "
    //    << pre_posts[i].post << endl;
  }
}
